...
int main()
{
	...
	IODIR1  = 0xFFFFFF00;	
	IO0SET = 0xFFFFFFFF; 				// LIMPIO TODAS LAS SALIDAS
	IO1SET = 0xFFFFFFFF;
	configTimer();
	configExcepcion();
  ...
  while(1)
  {
	...
	do{
		...
		{
		 boton_apretado_mov= esperar_mov();
			p2.res = movimiento(&p2,boton_apretado_mov,-1);
			if(p2.res == 1)
			{
				for(i = 0; i<3;i++){
					for(j=0;j<3;j++)
		...
		else
		{
			boton_apretado_sustr = esperar_mov();
			boton_apretado_mov= esperar_mov();
			p2.res = movimiento(&p2,boton_apretado_mov,boton_apretado_sustr);
			if(p2.res == 1)
			{
				for(i = 0; i<3;i++){
					for(j=0;j<3;j++)
		...
		for(j=0;j<3;j++){
			if(tabla[i][j]==1){
				if(!((3*i+j) == 3 || (3*i+j) == 5 || (3*i+j) == 6  ))
					IO0CLR |= jugador1[3*i+j];
				else
					IO1CLR |= jugador1[3*i+j];
			}
			if(tabla[i][j]==2)
				IO0CLR |= jugador2[3*i+j];
		}
	}
